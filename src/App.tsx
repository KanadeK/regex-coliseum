  ...
  return <main>
    ...
    <div className="layout">
      ...
      <section className="arena" aria-live="polite"><div className="arena-heading"><div><p className="eyebrow">{challenge.category}</p><h2>{challenge.title}</h2><p>{challenge.prompt}</p></div><button className="secondary" onClick={share}>Share challenge</button></div>
        <label>Pattern<input ref={sourceInput} value={source} onChange={(event) => setSource(event.target.value)} spellCheck="false" aria-describedby="pattern-help" /></label>
        ...
        <label>Flags<input value={flags} onChange={(event) => setFlags(event.target.value)} aria-describedby="flags-help" maxLength={8} /></label>
        <p id="flags-help" className="hint">Optional JavaScript flags, for example <code>i</code>.</p>
